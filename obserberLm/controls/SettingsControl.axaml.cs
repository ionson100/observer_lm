     ...
     bool Validate()
        {
            ...
            {
                MessageBoxManager.GetMessageBoxStandard("warning", "Token is  not UUID",ButtonEnum.Ok).ShowAsync();
                return false;
            }
            if (!Directory.Exists(TxtFolderPath.Text.Trim()))
            {
                MessageBoxManager.GetMessageBoxStandard("warning", "This is not an existing folder",ButtonEnum.Ok).ShowAsync();
                return false;
            }
